...
class PaystackPaymentController extends Controller
{
    public function getPaystackPaymentURL(PaystackPaymentURLRequest $request)
    {
        ...
        if(isset($request->crypto_type) && $request->crypto_type == CURRENCY_TYPE_FIAT){
            ...
            $query = '?coin_type='.$request->currency.'&crypto_type='.$request->crypto_type.'&amount='.$request->amount;
        }else{
            ...
            $query = '?coin_type=&crypto_type=1&wallet_id='.$request->wallet_id.'&amount='.$request->amount;
        }
