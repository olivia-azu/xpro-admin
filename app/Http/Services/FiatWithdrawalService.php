...
class FiatWithdrawalService extends BaseService
{
        ...
        try {
            ...
            if($wallet = Wallet::where(['user_id' => $userId, 'coin_type' => $request->currency])->first()){
                ...
                if($wallet->decrement('balance',$amount)){
                    $data = $this->makeWalletWithdrawalData($wallet, $coin, $bank, $userId, $request);
    ...
    public function makeWalletWithdrawalData($wallet, $coin, $bank, $userId, $request)
    {  
        return [
            "user_id" => $userId,
            "wallet_id" => $wallet->id,
    ...
    public function fiatWalletWithdrawalHistory($request)
    {
        ...
        $items->map(function ($item) {
            ...
            $item->bank_title = $item?->bank->bank_name;
        });
